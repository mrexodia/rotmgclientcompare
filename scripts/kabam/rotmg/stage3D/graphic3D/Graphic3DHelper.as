package kabam.rotmg.stage3D.graphic3D {
    ...
    public class Graphic3DHelper {
        ...
        private static function injectSingletonIndexBuffer(param1:Injector) : void {
            var _local_2:IndexBufferFactory = param1.getInstance(IndexBufferFactory);
            param1.map(IndexBuffer3DProxy).toProvider(_local_2);
        }
        
        private static function injectSingletonVertexBuffer(param1:Injector) : void {
            var _local_2:VertexBufferFactory = param1.getInstance(VertexBufferFactory);
            param1.map(VertexBuffer3DProxy).toProvider(_local_2);
        }
    }
}
