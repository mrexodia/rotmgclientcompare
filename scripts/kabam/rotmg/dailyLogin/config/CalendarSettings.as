package kabam.rotmg.dailyLogin.config {
    ...
    public class CalendarSettings {
        ...
        public static function getCalendarModalRectangle(param1:int, param2:Boolean) : Rectangle {
            var _local_3:int = Math.ceil(param1 / NUMBER_OF_COLUMNS);
            return new Rectangle(0,0,2 * DAILY_LOGIN_MODAL_PADDING + 2 * DAILY_LOGIN_TABS_PADDING + BOX_WIDTH * NUMBER_OF_COLUMNS + (NUMBER_OF_COLUMNS - 1) * BOX_MARGIN,TABS_HEIGHT + 2 * DAILY_LOGIN_TABS_PADDING + 2 * DAILY_LOGIN_MODAL_PADDING + BOX_HEIGHT * _local_3 + (_local_3 - 1) * BOX_MARGIN + DAILY_LOGIN_MODAL_HEIGHT_MARGIN + (!!param2?20:0));
        }
        
        public static function getTabsRectangle(param1:int) : Rectangle {
            var _local_2:int = Math.ceil(param1 / NUMBER_OF_COLUMNS);
